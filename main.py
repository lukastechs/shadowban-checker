...
async def check_search_ban_scraping(username: str):
    async with async_playwright() as p:
        ...
        try:
            ...
            tweets_found = await page.wait_for_selector('[data-testid="tweet"]', timeout=5000) is not None
...
async def check_ghost_ban(username: str):
    async with async_playwright() as p:
        ...
        try:
            ...
            reply_tweet = await page.wait_for_selector('[data-testid="tweet"]', timeout=5000) is not None
            if not reply_tweet:
                return "Unable to detect (no recent replies)"
            reply_url = await page.evaluate("""
                () => {
                    const link = document.query_selector('[data-testid="tweet"] a[href^="/"][href*="status"]');
    ...
    async with async_playwright() as p:
        ...
        try:
            ...
            if not reply_url:
                return "Unable to detect (could not extract reply URL)"
            await page.goto(reply_url, wait_until="networkidle", timeout=30000)
            show_more = "Show more replies" in await page.evaluate("document.body.innerText")
...
@app.get("/")
async def root():
    return {"message": "X Shadow Ban Checker API is running"}

@app.post("/check")
async def check_shadow_ban(request: CheckRequest):
    async with aiohttp.ClientSession() as session:
        try:
            async with session.post(
                ...
                recaptcha_data = await resp.json()
...
async def check_shadow_ban_test(username: str):
...
@app.get("/health")
async def health():
    return {"status": "healthy", "timestamp": os.popen("date -u").read().strip()}
